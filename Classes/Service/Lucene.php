...
class Lucene extends \TYPO3\CMS\Core\Service\AbstractService implements \TYPO3\CMS\Core\SingletonInterface
{
  ...
  public function query($searchTerm)
  {
    ...
    if (strlen(trim($searchTerm))) {
      ...
      foreach (\Tollwerk\TwLucenesearch\Utility\Indexer::indexConfig($GLOBALS['TSFE'], 'search.searchTypes') as $searchType) {
        ...
        if ($searchType == Indexer::PAGE) {
          ...
          $pageQueries = array(new \Zend_Search_Lucene_Search_Query_Term(new \Zend_Search_Lucene_Index_Term($searchType, 'type')));
          ...
          if (\Tollwerk\TwLucenesearch\Utility\Indexer::indexConfig($GLOBALS['TSFE'], 'search.restrictByLanguage')) {
            $pageQueries[] = new \Zend_Search_Lucene_Search_Query_Term(new \Zend_Search_Lucene_Index_Term($GLOBALS['TSFE']->lang, 'language'));
          }
