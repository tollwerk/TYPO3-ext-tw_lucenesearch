...
class Indexer implements \TYPO3\CMS\Core\SingletonInterface
{
    ...
    if (!intval($fe->page['no_search']) && self::indexConfig($fe, 'enable')) {
      ...
      if ($indexerService instanceof \TYPO3\CMS\Core\Service\AbstractService) {
        ...
        if ($document instanceof \Zend_Search_Lucene_Document) {
          ...
          if ((!$this->_debug || !array_key_exists('index_force_reindex',
                $_GET) || !intval($_GET['index_force_reindex'])) && $timestamp && ($timestamp <= intval($document->getField('timestamp')->value))
          ) {
            return;
   ...
   */
  protected function _getMetaContent(
    \TYPO3\CMS\Frontend\Controller\TypoScriptFrontendController $fe,
    $meta,
    &$metaTag = null
  ) {
    ...
    if (strlen($meta) && preg_match('%\<meta([^\>]+?)name\=(\042|\047)'.addcslashes($meta,
          '.?+*[]{}()').'\\2([^\>]*?)\>%i', $fe->content, $metaTag) && preg_match("%content\=(\042|\047)(.*?)\\1%i",
  ...
  public static function indexConfig(\TYPO3\CMS\Frontend\Controller\TypoScriptFrontendController $fe, $key = null)
  {
    if (self::$_config === null) {
      self::$_config = self::indexConfigTS((isset($fe->tmpl) && ($fe->tmpl instanceof \TYPO3\CMS\Core\TypoScript\TemplateService) && is_array($fe->tmpl->setup) && array_key_exists('config.',
    ...
    if (strlen($indexReference) || strlen($linkVars)) {
      ...
      if (array_key_exists('search_lucene.', $typoscript) && is_array($typoscript['search_lucene.'])) {
        ...
        if (array_key_exists('searchTypes',
          ...
          $config['search']->searchTypes = array_filter(array_map('trim', explode(',', $typoscript['search_lucene.']['searchTypes'])));
        }
        if (array_key_exists('searchConfig',
          ...
          foreach (\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',
            ...
            if (preg_match("%^([^\:]+)\:([^\^\~]+)(\~(\d+(?:\.\d+)?)?)?(?:\^(\d+(?:\.\d+)?))?$%", $searchConfig,
              $searchConfigParams)) {
